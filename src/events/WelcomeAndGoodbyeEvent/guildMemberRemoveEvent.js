...
module.exports = {
    ...
    async execute(member) {
        ...
        const guildData = await GoodbyeMessage.findOne({ guildId: member.guild.id });
        if (!guildData) return;
        ...
        // Load background
        const background = await Canvas.loadImage("https://i.postimg.cc/DwNqcd3K/Testi-9.png"); // Ganti dengan URL gambar goodbye
        ...
        context.fillText("GOODBYE", 512, 360);

        context.font = "42px sans-serif";
        context.fillText(member.user.username.toUpperCase(), 512, 410);

        context.font = "32px sans-serif";
        context.fillText(`We now have ${addSuffix(member.guild.memberCount)} members`, 512, 455);

        // Create attachment
        const attachment = new AttachmentBuilder(canvas.toBuffer(), { name: `goodbye-${member.id}.png` });
